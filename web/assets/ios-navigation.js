(() => {
  ...
  function showSection(kind,push=true){
    ...
    page.innerHTML=`<div class="fx-section-aurora a"></div><div class="fx-section-aurora b"></div><header><div><p>ForwardX</p><h1>${g.title}</h1><span>${g.sub}</span></div><div class="fx-header-orb">${glyph[kind]}</div></header><section class="fx-page-grid ${kind}">${g.items.map(item).join('')}</section>`;
  ...
  function removeSection(){document.getElementById('fx-section-page')?.remove()}
  ...
  function tabs(){if(document.getElementById('fx-ios-tabbar'))return;const nav=document.createElement('nav');nav.id='fx-ios-tabbar';nav.innerHTML=[['dashboard','仪表盘'],['forward','转发'],['manage','管理'],['me','我的']].map(x=>`<button data-tab="${x[0]}"><span>${glyph[x[0]]}</span><b>${x[1]}</b></button>`).join('')+'<i class="fx-tab-lens"></i>';document.body.appendChild(nav);nav.querySelectorAll('button').forEach(b=>b.onclick=()=>tab(b.dataset.tab));mark()}
  ...
  function boot(){if(!isIOS())return;document.documentElement.classList.add('fx-ios-native');tabs();routeChanged();new MutationObserver(()=>{tabs();if(!currentVirtual)dashboardGrid();mark()}).observe(document.getElementById('root')||document.body,{childList:true,subtree:true});addEventListener('popstate',()=>setTimeout(routeChanged,40))}
