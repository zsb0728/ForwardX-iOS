...
struct LoginView: View {
    ...
    var body: some View { ScrollView { VStack(spacing:28) {
        ...
        VStack(spacing:6){Text("ForwardX").font(.title.bold());Text("掌控每一条链路").foregroundStyle(.secondary)}
...
struct MainShell: View {
    ...
    var body: some View { TabView {
        ...
        NavigationStack { FeatureCenterView(store:store) }.tabItem{Label("功能",systemImage:"square.grid.3x3.fill")}
...
struct DashboardView: View {
    ...
    var body: some View { ScrollView { VStack(spacing:12) {
        HStack { VStack(alignment:.leading){Text("欢迎回来").foregroundStyle(.secondary);Text(store.user.text("name","username")).font(.title.bold())};Spacer();Button{Task{await store.refresh()}}label:{Image(systemName:"arrow.clockwise").padding(12).modifier(GlassButtonModifier())} }
        ...
        GlassCard { VStack(alignment:.leading,spacing:10){Label("网络态势",systemImage:"waveform.path.ecg").font(.headline);HStack{StatusPill(online:true);Text("\(store.hosts.count) 台主机 · \(store.tunnels.count) 条链路").foregroundStyle(.secondary);Spacer()};ProgressView(value:store.hosts.isEmpty ? 0:Double(store.hosts.filter{$0.data.flag("online","isOnline")}.count),total:max(1,Double(store.hosts.count))).tint(.green)} }
...
struct Quick:View{let icon,title:String;let color:Color;var body:some View{VStack(spacing:8){Image(systemName:icon).font(.headline).foregroundStyle(color).frame(width:48,height:48).background(color.opacity(0.12),in:Circle());Text(title).font(.caption)}.frame(maxWidth:.infinity)}}
