...
struct ItemsView: View {
    ...
    var createOperation: FXOperation { FXOperation(path: kind == .hosts ? "hosts.create" : kind == .rules ? "rules.create" : "tunnels.create", mutation: true) }
    var body: some View { ScrollView { LazyVStack(spacing:10) {
        ...
        ForEach(filtered) { item in switch kind {case .hosts:HostCard(store:store,item:item);case .rules:RuleCard(store:store,item:item);case .tunnels:TunnelCard(store:store,item:item)} }
    }.padding() }.navigationTitle(kind.title).searchable(text:$search,prompt:"搜索").refreshable{await store.refresh()}.toolbar{ToolbarItem(placement:.topBarTrailing){NavigationLink { OperationView(store:store,operation:createOperation) } label:{Image(systemName:"plus").padding(8).modifier(GlassButtonModifier())}}} }
}

struct HostCard: View { @Bindable var store:ForwardXStore; let item:FXItem
    var online:Bool { item.data.flag("online","isOnline","agentOnline") }
    var body:some View { NavigationLink { ManageDetailView(store:store,kind:.hosts,item:item) } label:{ GlassCard { VStack(alignment:.leading,spacing:10){HStack{Image(systemName:"server.rack").font(.headline).foregroundStyle(.blue);VStack(alignment:.leading){Text(item.data.text("name")).font(.headline).foregroundStyle(.primary);Text(item.data.text("address","ip","publicIp")).font(.caption).foregroundStyle(.secondary)};Spacer();StatusPill(online:online)};Divider().opacity(0.4);HStack{Mini(label:"CPU",value:"\(Int(item.data.num("cpuUsage","cpu")))%");Mini(label:"内存",value:"\(Int(item.data.num("memoryUsage","memory")))%");Mini(label:"流量",value:bytes(item.data.num("trafficOut","totalTraffic")))} } } }.buttonStyle(.plain) }
}

struct RuleCard:View { @Bindable var store:ForwardXStore;let item:FXItem
    var active:Bool{item.data.flag("isEnabled","enabled")}
    var body:some View{NavigationLink { ManageDetailView(store:store,kind:.rules,item:item) } label: { GlassCard{VStack(alignment:.leading,spacing:10){HStack{ZStack{Circle().fill((active ? Color.green:Color.gray).opacity(0.13));Image(systemName:"arrow.left.arrow.right").foregroundStyle(active ? .green:.secondary)}.frame(width:38,height:38);VStack(alignment:.leading){Text(item.data.text("name","remark")).font(.subheadline.bold());Text("\(item.data.text("listenPort","port")) → \(item.data.text("targetAddress","targetHost")):\(item.data.text("targetPort"))").font(.caption2).foregroundStyle(.secondary)};Spacer();Toggle("",isOn:Binding(get:{active},set:{_ in Task{await store.toggleRule(item)}})).labelsHidden()};HStack{Text(item.data.text("protocol","type").uppercased()).font(.caption2.bold()).padding(.horizontal,8).padding(.vertical,4).background(.blue.opacity(0.12),in:Capsule());Spacer();Text(bytes(item.data.num("trafficIn")+item.data.num("trafficOut"))).font(.caption2).foregroundStyle(.secondary)}}} }.buttonStyle(.plain)
}
struct TunnelCard:View{@Bindable var store:ForwardXStore;let item:FXItem;var active:Bool{item.data.flag("isEnabled","enabled","available")};var body:some View{NavigationLink{ManageDetailView(store:store,kind:.tunnels,item:item)}label:{GlassCard{VStack(alignment:.leading,spacing:10){HStack{Image(systemName:"point.3.connected.trianglepath.dotted").font(.headline).foregroundStyle(.purple);VStack(alignment:.leading){Text(item.data.text("name")).font(.headline).foregroundStyle(.primary);Text(item.data.text("type","protocol")).font(.caption).foregroundStyle(.secondary)};Spacer();StatusPill(online:active)};HStack{Text(item.data.text("entryHostName","entryAddress")).lineLimit(1);Image(systemName:"arrow.right").foregroundStyle(.secondary);Text(item.data.text("exitHostName","exitAddress")).lineLimit(1)}}}}.buttonStyle(.plain)}}
